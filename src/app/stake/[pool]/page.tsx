...
const blockAverageInterval = (
    ...
    const count = result.data?.blocks?.length as number;
    if (count > 0) {
        const last = result.data?.blocks[count - 1] as Block;
        const now = Date.now();
...
const PoolInfo = ({ formattedAddress }: ENSStaticProps) => {
    ...
    // query pool data
    const { amount, amounts, rebalance } = useStakingPool(
        address,
        account as string
    );
    ...
    const isManager = Boolean(
        account && account.toLowerCase() === stakingPool?.manager
    );
